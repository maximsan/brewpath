...
- **Never cite `prototype/` from `lib/`** — no file names, no line numbers. The
  ...
  surface)`, `padding: 26px 24px`): it is greppable, it survives a re-drop, and
  it is the part that makes a literal falsifiable rather than arbitrary. Cite
  the **ruling** (ADR, issue) for a decision — `docs/README.md`'s precedence
  rule has rulings beat the prototype. Locations are fine as scaffolding while
