...
**All three reward screens are two-phase.** They open on `phase: 'roasty'`
...
| `ModuleCompleteScreen` | `module`, `autoMs={2200}` | `MODULE COMPLETE`, "Look how far you've come." | `onContinue` · `onBack` · **`flipTo(true/false)`** — the reward card flips |
| `ModuleRewardCardScreen` | `card` | `REWARD UNLOCKED`, "You earned a card." | `onContinue` · `onBack` |
