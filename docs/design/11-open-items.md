...
- ✅ **The three duplicate collectible titles are resolved** — Extraction, Fermentation and The Cherry in Section. Fixed structurally: visual guides moved out of `COLLECTION` into their own `VISUAL_GUIDE_CARDS` array, because sharing one array *"made a plain 'no two cards share a title' check report false collisions."* QA now carries the check as a standing invariant.
...
| ~~**Whether a replay ticks the streak is unspecified**~~ ✅ **RESOLVED** | Help FAQ vs [§5](05-mechanics.md) 5.4 | **A completed replay counts**, once it reaches the final card — product-owner ruling (`docs/decisions.md` §3), recorded at [The streak's qualifying-activity rule](https://github.com/maximsan/brewpath/issues/33). Replays still pay no points and no tree growth; they feed the streak only. The post-course horizon is answered separately: **Keep Sharp** becomes the Today recommendation after Foundations, and more modules / the Atlas are explicitly *not* the retention answer. ⚠️ The Help FAQ string at `settings.jsx:631` still says *"finish at least one lesson a day"* and now understates the rule — copy fix owed |
| **Streak advancement is unwired, by design** | `app.jsx:605–630` | Not a defect — the missing clock is documented in-source, including the correct real-world form (`heldBefore < FREEZE_CAP` rather than the derived value). Listed so a porter knows to wire the day rollover and use the pre-earn count. The cap constant matches shipping (**1**) since the authorized surface edit ([#196](https://github.com/maximsan/brewpath/issues/196)); ⚠️ the **derivation** is still not portable — §10 rules no accrual while holding and 7 fresh days after each spend ([Streak freeze](https://github.com/maximsan/brewpath/issues/58)), while the prototype's clamp re-grants from the streak total. Because `setFreezesSpent` is called only in reset paths, **no freeze has ever been earned or spent in the prototype** — unexercised, not observed behaviour |
...
| **The vocab game uses a biased shuffle; flashcards twelve lines away use a correct one** | `dictionary-extras.jsx:279–284` vs `:135` | Vocab picks terms, distractors and choice order with `[...src].sort(() => Math.random() - 0.5)`. That idiom is **not** a uniform shuffle — comparator-based randomisation gives a measurably skewed distribution, so some terms surface far more often than others in the surface whose entire job is variety. Flashcards use real Fisher–Yates at `:135`. **Port the Fisher–Yates one to both**; the shuffle helper [#66](https://github.com/maximsan/brewpath/issues/66) adds for mini-game round order is the natural home |
| **The vocab game's results line says "added" on the deck the terms were already in** | `dictionary-extras.jsx:432` — the `tail` string | The line reads *"The N terms you missed were added to your review deck"*, unconditionally. It is false whenever the drill was **drawn from** the Misses deck — which is the deck the line is read on most: those terms were already in it and stayed. **The app diverges deliberately**: the verb follows the deck the drill came from, *kept in* rather than *added to* ([#298](https://github.com/maximsan/brewpath/issues/298)) |
| **Mini-game round order is fixed, and the player runs the whole bank** | `lesson.jsx:1340` | `content[roundIndex]` across the full array, so a run is always the same rounds in the same sequence — for a free user, the identical 11 rounds forever. **Resolved by decision** at [#66](https://github.com/maximsan/brewpath/issues/66): a seeded per-run permutation across all seven games, following [#11](https://github.com/maximsan/brewpath/issues/11)'s pure-seeded pattern. Recorded as **cosmetic** — it does not add content, and was taken only because it costs nothing |
...
| Item | Detail |
|---|---|
| **The card cue sits taller than the design's** | The design pulls its `?` back onto the cue line with `margin: '-13px -8px'`, so a 44px touch area does not set the height of a one-line eyebrow. CSS hit-tests the padding box regardless of layout size; Flutter rejects a tap outside the parent's own box, so reproducing the bleed (`OverflowBox`) would silently shrink the target to 28×18. The 44 target is kept and the cue line is taller by that much — [#550](https://github.com/maximsan/brewpath/issues/550) |
| **`match`'s cue says *drag to pair*, and the board is tap-then-tap** | The phrase is the design's, whose board carries both drag and tap; the app's board ([#550](https://github.com/maximsan/brewpath/issues/550)) only taps. The drawer's own blurb offers both — *"or tap one then the other"* — so the instruction is reachable, but the cue names an interaction the app does not have. Either the board gains dragging or the phrase drops the verb; not ruled |
