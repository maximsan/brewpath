...
| `assets/modules/m1–m5.png`, bundled as `assets/modules/m1–m5.jpg` | 5 | The module pictures Today's card carries under the lesson title, and the dictionary's *Where you'll learn it* row shows as a 48pt square. Here they stay lossless at 1254×1254, ~11.7 MB together; the app bundles them re-encoded as 1100×1100 JPEGs, **~0.88 MB together** ([#540](https://github.com/maximsan/brewpath/issues/540)). The extractor rewrites the extension when it writes the modules bank, so the bank names what actually ships and this source is never edited for a shipping concern. Both stay square — `artPos` anchors a crop of the whole picture. **With the trees, the only production raster assets.** |
