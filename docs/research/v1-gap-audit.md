...
4. **The content is a different course, and roughly a third the size.** 25 five-step lessons
   ...
   all (`assets/content/modules.json`, `lessons.json`). Dictionary (42 terms), brew challenges
...
| 9 brew challenges (`BREW_TOTAL`, verified `brew-challenge.jsx:20-61`) | **0** | missing. Note §11 line says "10 brew challenges" — the source says 9; source wins |
...
| Reward routing incl. "next lesson only if authored" fallback | **partially present** | Module-complete routes to a `moduleSummary` recap (`lesson_completion_body.dart:64-71`, `app_router.dart:136-143`); otherwise Continue goes to `/learn`. **No** "next lesson if authored, else Path" chaining, no `module-card` step, no perfect branch, no brew-challenge offer. |
...
| 9 brew challenges (§11 says 10; source says 9) | **missing** | — |
...
| Saved free cap 10 → gate sheet, removal always allowed | **missing** | No cap, no gate, no Saved screen (see Favourites above). |
