...
const MODULES = [
  ...
  { id: 'm5', n: 5, label: 'BREW',      title: 'Brew',      glyph: 'brewing', art: 'assets/modules/m5-brew.png', artPos: '50% 56%', locked: true, lessons: [
      ...
      { id: 'm5l3', title: 'Tasting your cup',   points: 10, time: 6, status: 'locked' },
      { id: 'm5l7', title: 'Espresso, in one small cup', points: 10, time: 5, status: 'locked' },
      { id: 'm5l6', title: 'Your first good cup', points: 10, time: 5, status: 'locked' },
  ]},
];

// Lessons with full card payloads. Every lesson listed in MODULES is authored here.
const LESSONS = {
  m1l1: {
    ...
    cards: [
      ...
      { kind: 'mcq',
        prompt: 'The fruit stripped off the seed \u2014 what happens to it?',
        choices: [
          { t: 'Dried and sold as a tea called cascara', correct: true },
          { t: 'Roasted along with the seeds' },
          { t: 'Pressed into coffee oil' },
          { t: 'Nothing \u2014 it is not edible' },
        ],
        explain: 'Cascara is the dried cherry skin, brewed like a fruit tea \u2014 sweet, and nothing like coffee. It is the neatest proof of this lesson: the part most of the world throws away is the part you can actually eat.',
      },
      { kind: 'mcq',
  ...
  m1l2: {
    ...
    cards: [
      ...
      { kind: 'tastefix',
        tags: ['THIN', 'LOST IN MILK'],
        prompt: 'Your flat white tastes like warm milk. What would you change first?',
        scenario: 'The same beans pull a sweet, bright shot when you drink it straight.',
        choices: [
          { t: 'Move to a blend with some Robusta', correct: true },
          { t: 'Switch to a lighter roast' },
          { t: 'Froth the milk hotter' },
          { t: 'Pull a longer shot' },
        ],
        explain: 'Milk flattens exactly the delicate acidity that makes a bright Arabica shot lovely on its own. Body and crema are what survive it — which is why classic Italian espresso blends keep 10–20% Robusta. A lighter roast goes the wrong way: more delicate still, and more lost.',
      },
      { kind: 'recall', label: 'BEFORE YOU GO',
  ...
  m1l3: {
    ...
    cards: [
      { kind: 'predict', label: 'LESSON 3', title: 'What origin means',
        ...
        question: 'Same variety, same roast, two different mountains. Same cup?',
        options: ['Roughly the same', 'Two different cups'],
        a: 'Two different cups',
      ...
      { kind: 'concept', label: 'CONCEPT', title: 'Origin is a place',
        ...
        paragraphs: [
          ...
          'The more precisely a bag names its origin, the more the roaster is telling you they know exactly where it came from. Coffee borrowed wine’s word for why that matters: terroir — soil, altitude and climate leaving a signature no other place can copy.',
        ],
      ...
      { kind: 'mcq',
        prompt: 'A bag says only “Brazil”. What can you safely predict about the cup?',
        choices: [
          { t: 'Very little — that is thousands of farms', correct: true },
          { t: 'It will be sweet and nutty' },
          { t: 'It was grown above 1,500 m' },
          { t: 'It was dried whole, in the cherry' },
        ],
        explain: 'Brazil grows coffee across an area the size of western Europe, at every altitude and by every process there is. Origin signatures are real tendencies worth learning, but a country line is the broadest possible version of the claim — sweet and nutty is a fair guess at Brazil and nothing more than a guess. The narrower the origin, the more it actually predicts.',
      },
      { kind: 'multi',
        ...
        choices: [
          ...
          { t: 'The year it was harvested' },
        ],
        explain: 'Origin is about place — country, region, farm, altitude. Harvest year sits right beside those on the bag and matters a great deal for freshness, but it says when the coffee was picked, not where it came from. Roast level is further removed still: something a roaster did months later and, usually, a continent away.',
      },
      { kind: 'match',
  ...
  m1l4: {
    ...
    cards: [
      ...
      { kind: 'match',
        prompt: 'Match each trait to the altitude band it points to',
        pairs: [
          { l: 'Soft, neutral cup',        r: 'Below 1,000 m' },
          { l: 'Balanced, sweet cup',      r: '1,200–1,600 m' },
          { l: 'Bright, complex cup',      r: 'Above 1,800 m' },
      ...
      { kind: 'decision', label: 'AT THE SHELF', title: 'The number in metres',
        ...
        options: [
          ...
          { t: 'The bag with no altitude', sub: 'No altitude printed' },
        ],
  ...
  m1l6: {
    ...
    cards: [
      ...
      { kind: 'visual', label: 'VISUAL GUIDE', title: 'The variety family tree', visualGuide: 'variety',
        caption: 'When a bag prints a variety, this is the line it is pointing at — not the country, not the process.' },
      { kind: 'mcq',
        prompt: 'One of these lines names the variety. Which?',
        choices: [
          { t: 'Bourbon', correct: true },
          { t: 'Sidamo' },
          { t: 'Honey' },
          { t: 'Strictly high grown' },
        ],
        explain: 'Bourbon is a cultivar \u2014 one of the two ancestors from the family tree. It reads like a place because it was one: the island of Bourbon, now R\u00e9union, where it was first grown. Sidamo is the region, honey is a process, and strictly high grown is an altitude grade.',
      },
      { kind: 'decision', label: 'AT THE SHELF', title: 'Same everything, one line apart',
        ...
        options: [
          ...
          { t: 'The heirloom', sub: 'Blend, no variety named' },
        ],
        ...
        note: 'To learn what a variable does, change that variable and nothing else.' },
      { kind: 'recall', label: 'BEFORE YOU GO',
  ...
  m1l7: {
    ...
    cards: [
      ...
      { kind: 'visual', visualGuide: 'anatomy', label: 'VISUAL GUIDE', title: 'Six layers, outside in', captionTop: true,
      ...
      { kind: 'concept', label: 'CONCEPT', title: 'What the seed still wears',
        ...
        paragraphs: [
          'Coffee rests and ships inside its parchment, then gets hulled just before export.',
          'Coffee ships inside its parchment and is hulled just before export, which is why green coffee reaches a roastery bare. The silverskin is the last tissue-thin membrane: most of it flakes off in the roaster as chaff, and what survives stays packed in the bean’s centre crease.',
        ],
        meta: [['LAYER 04', 'Parchment'], ['LAYER 05', 'Silverskin · chaff'], ['SHIPS AS', 'Green coffee']],
      },
      { kind: 'sequence',
    ...
    reward: {
      ...
      meta: [
        ['LAYERS', '6'],
        ['THE STICKY ONE', 'Mucilage'],
        ['BECOMES CHAFF', 'Silverskin'],
      ],
    },
  },
  ...
  m2l1: {
    ...
    cards: [
      ...
      { kind: 'match',
        prompt: 'Match each clue to its process',
        pairs: [
          { l: 'Fruit removed before drying', r: 'Washed' },
          { l: 'Dried whole in the cherry',   r: 'Natural' },
          { l: 'Some pulp left on the seed',  r: 'Honey' },
          { l: 'Biggest berry-fruit flavour', r: 'Natural' },
        ],
      },
      { kind: 'mcq',
        prompt: 'What does the \u201cwashed\u201d in washed process actually refer to?',
        choices: [
          { t: 'Rinsing the fruit off the seed before drying', correct: true },
          { t: 'Rinsing the beans after roasting' },
          { t: 'Hosing the cherries down before picking' },
          { t: 'Soaking the green beans to soften them' },
        ],
        explain: 'The washing happens at the farm, to the wet seed, before it dries. Nothing is rinsed after roasting \u2014 that would strip the oils the roast just built, and a roasted bean has no fruit left on it to remove.',
      },
      { kind: 'multi',
  ...
  m2l2: {
    ...
    cards: [
      ...
      { kind: 'mcq',
        prompt: 'Where do the boozy, wine-like notes in a natural come from?',
        choices: [
          { t: 'Fruit sugars fermenting as it dries', correct: true },
          { t: 'Barrels the green beans rested in' },
          { t: 'A darker roast on the same lot' },
          { t: 'The variety that was planted' },
        ],
        explain: 'The fruit is still on the seed while it dries, and its sugars ferment in place \u2014 that is the whole source. Barrel-ageing is a real thing some roasters do, but it is a separate trick applied afterwards, not what makes a natural taste like a natural.',
      },
      { kind: 'sequence',
      ...
      { kind: 'decision', label: 'AT THE SHELF', title: 'Your favourite bag is sold out',
        scenario: 'You’ve been buying one bag on repeat for the jammy, boozy cup it makes: a natural from a farm in Ethiopia. Today the shop is out of it.',
        question: 'Which substitute gets you closest to that cup?',
        options: [
          { t: 'A natural from Brazil', sub: 'Different country · strawberry, rum', correct: true },
          { t: 'A washed lot from that same Ethiopian farm', sub: 'Same farm, same variety · lemon, tea' },
        ],
        right: 'Different mountain, same fermentation. The fruit stayed on the seed while it dried, and that is what made the cup you liked — it travels better than the farm name does.',
        wrong: 'Tempting, because everything else on the label matches. But strip the fruit off before drying and the jam has nowhere to come from — you get a clean, citrusy coffee that shares an address with your favourite and nothing else.',
        note: 'When you chase a flavour, match the process line before you match the origin line.' },
      { kind: 'recall', label: 'BEFORE YOU GO',
    ...
    cards: [
      ...
      { kind: 'match',
        ...
        pairs: [
          ...
          { l: 'Lot number printed',  r: 'Very specific origin' },
        ],
      },
      { kind: 'multi',
        ...
        explain: 'Process, origin and tasting notes all forecast the cup. A far-off \u201Cbest by\u201D date and the price tell you nothing about how it will taste.',
      },
      { kind: 'mcq',
        prompt: 'A bag reads \u201CKenya \u00B7 Washed \u00B7 blackcurrant, grapefruit.\u201D Expect\u2026',
        choices: [
          { t: 'A bright, juicy, high-acid cup', correct: true },
          { t: 'A sweet, jammy, low-acid cup' },
          { t: 'A dark, smoky, roast-forward cup' },
          { t: 'A soft, mild cup with very little acidity' },
        ],
        explain: 'Blackcurrant and grapefruit are acid words, not jam words \u2014 and washed keeps that acidity clean and lit up. Fruit on a label does not always mean jammy: it matters which fruit.',
      },
      { kind: 'decision', label: 'AT THE SHELF', title: 'Read the label like a forecast',
        scenario: 'You want something sweet and fruity for a slow weekend brew. Two bags promise it. One names a farm you’ve read about; the other names only the country.',
        question: 'Which label is the safer buy?',
        options: [
          { t: 'Colombia · Natural', sub: 'no farm named · plum, cocoa · roasted 9 days ago', correct: true },
          { t: 'Ethiopia · Gedeb Estate · Natural', sub: 'famous farm · blueberry, syrupy · best by March 2027' },
        ],
        right: 'Both forecast the cup you want, so the process line is a wash and the roast date decides it. Nine days ago is a real date; the plainer bag is the one you can trust.',
        wrong: 'The best label on the shelf, except for the line that matters most here. “Best by” is not a roast date — it hides how long the bag sat before it reached you, and stale coffee tastes flat whatever the farm is called.',
        note: 'A named farm is a promise about the coffee. A roast date is a promise about the bag in your hand.' },
      { kind: 'recall', label: 'BEFORE YOU GO',
        question: 'A bag says only “Blend · Dark Roast · best by June 2027.” What does that label tell you?',
        choices: [
          { t: 'Very little', correct: true },
  ...
  m2l4: {
    ...
    cards: [
      ...
      { kind: 'match',
        prompt: 'Match each trait to the drying method',
        pairs: [
          { l: 'Airflow on every side',           r: 'Raised beds' },
          { l: 'Sun-warmed concrete underfoot',   r: 'Patios' },
          { l: 'Heated drums for speed',          r: 'Mechanical dryers' },
          { l: 'The fallback when rain won’t stop', r: 'Mechanical dryers' },
        ],
      },
      { kind: 'mcq',
      ...
      { kind: 'decision', label: 'AT THE SHELF', title: 'Drying you can taste',
        ...
        options: [
          { t: 'Someone was close enough to the lot to record it', correct: true },
          { t: 'The drying was slower than most naturals get' },
          { t: 'The cup will be sweeter than a patio-dried lot' },
        ],
        ...
        wrong: 'Twenty-two days is slow, and a slow dry does tend to build sweetness — but the bag is not promising you either. What it actually proves is that somebody was close enough to the drying beds to count the days, which is a claim about the supply chain rather than the cup.' },
      { kind: 'recall', label: 'BEFORE YOU GO',
  ...
  m2l5: {
    ...
    cards: [
      ...
      { kind: 'multi',
        ...
        choices: [
          { t: 'Sealed tanks change which microbes work', correct: true },
  ...
  m2l6: {
    ...
    cards: [
      ...
      { kind: 'decision', label: 'IN THE KITCHEN', title: 'Nine o’clock, and someone says “chemicals”',
        ...
        options: [
          { t: 'Pour it and explain the processes', sub: 'Decaf, 9pm', correct: true },
          { t: 'Tip it out and make regular coffee', sub: 'Regular, 9pm' },
        ],
  ...
  m3l1: {
    ...
    cards: [
      ...
      { kind: 'match',
        prompt: 'Match each taste to the roast level',
        pairs: [
          { l: 'Bright, fruity, acidic',      r: 'Light' },
          { l: 'Balanced, sweet',             r: 'Medium' },
          { l: 'Bitter, smoky, bold',         r: 'Dark' },
          { l: 'Origin character out front',  r: 'Light' },
        ],
      },
      { kind: 'mcq',
      ...
      { kind: 'decision', label: 'AT THE SHELF', title: 'A friend\u2019s first bag',
        ...
        note: 'Start at medium. Lean lighter or darker once you know which way you want to go.' },
      { kind: 'recall', label: 'BEFORE YOU GO',
  ...
  m3l2: {
    ...
    cards: [
      ...
      { kind: 'mcq',
        prompt: 'Why do very dark beans look shiny and oily?',
        choices: [
          { t: 'Oils have been pushed to the surface', correct: true },
          { t: 'Sugar has caramelised on the outside' },
          { t: 'They draw moisture back from the air' },
          { t: 'The roaster sprays them to keep them fresh' },
        ],
        explain: 'Push a bean far enough and its structure opens up, letting the oils inside migrate out to the surface. That sheen is the roast level showing itself, not a coating anyone applied \u2014 and it is why very dark beans stale faster than light ones, with their oils sitting exposed to the air. Caramelising sugar browns a bean; it does not make it wet.',
      },
    ...
    cards: [
      ...
      { kind: 'decision', label: 'AT THE SHELF', title: 'What to do at the shelf',
        ...
        options: [
          ...
          { t: 'Buy all three', sub: 'Three bags, one price' },
        ],
  ...
  m3l5: {
    ...
    cards: [
      ...
      { kind: 'match',
        prompt: 'Match each sign to the roast that shows it',
        pairs: [
          { l: 'Floral and fruity',          r: 'Light roast' },
          { l: 'Caramel and nutty',          r: 'Medium roast' },
          { l: 'Smoky and bittersweet',      r: 'Dark roast' },
          { l: 'An oily sheen on the bean',  r: 'Dark roast' },
        ],
      },
      { kind: 'sequence',
      ...
      { kind: 'decision', label: 'AT THE SHELF', title: 'Buying for the brewer you own',
        scenario: 'You drink one flat white a day and nothing else. A friend offers you either of two bags: a delicate light-roast Ethiopian, or a medium-dark Brazilian.',
        question: 'Which do you take?',
        options: [
          { t: 'The Brazilian', sub: 'Medium-dark', correct: true },
          { t: 'The Ethiopian', sub: 'Light roast' },
        ],
        right: 'Milk buries delicate acidity and floral aroma. A darker roast brings the caramel and cocoa that survive it, which is why café house blends sit where they do.',
        wrong: 'It is the better coffee on paper, and under steamed milk you would taste almost none of what you paid for.',
        note: 'Neither roast is better. Match it to how you drink it: black filter rewards light, milk rewards darker.' },
      { kind: 'recall', label: 'BEFORE YOU GO',
  ...
  m3l6: {
    ...
    cards: [
      ...
      { kind: 'visual', label: 'VISUAL GUIDE', title: 'Caffeine, per serving', visualGuide: 'caffeine',
        caption: 'Find the one you drink most often, then read across.' },
      { kind: 'slider',
      ...
      { kind: 'decision', label: 'IN THE KITCHEN', title: 'Four o’clock',
        ...
        options: [
          ...
          { t: 'Switch to a dark roast', sub: 'Dark roast, same scoop' },
        ],
    ...
    cards: [
      ...
      { kind: 'decision', label: 'IN THE KITCHEN', title: 'Match the grind to the brewer first',
        ...
        options: [
          ...
          { t: 'Nudge it one step coarser', sub: 'One step coarser' },
        ],
      ...
      { kind: 'recall', label: 'BEFORE YOU GO',
        ...
        choices: [
          { t: 'Finer particles expose more surface', correct: true },
  ...
  m4l2: {
    ...
    cards: [
      { kind: 'predict', label: 'LESSON 2', title: 'Burr vs blade',
        ...
        question: 'A blade grinder’s real problem is that it makes grounds ___',
        options: ['Too fine', 'All different sizes'],
        a: 'All different sizes',
      ...
      { kind: 'visual', label: 'VISUAL GUIDE', title: 'One setting, many sizes', visualGuide: 'distribution',
        caption: 'No grinder makes one single size \u2014 even a good burr produces a spread. It is just a narrow enough one to dial in.' },
      { kind: 'tastefix', tags: ['BITTER', 'FLAT'],
        prompt: 'Bitter all the way through, on a grinder you trust. What now?',
        scenario: 'Fresh beans, right ratio, water just off the boil, and a burr grinder that has never given you trouble. The bitterness is even, all the way through.',
        choices: [
          { t: 'Grind coarser', correct: true },
          { t: 'Buy a better grinder' },
          { t: 'Grind finer' },
          { t: 'Brew with hotter water' },
        ],
        explain: 'Even bitterness is a dial problem, not a grinder problem: uniform grounds simply gave the water too much surface to work on, so go coarser and it pulls less. Buying a grinder is the answer to a different fault \u2014 bitter and thin in the same sip, where dust over-extracts while boulders barely extract at all. One flavour, evenly, is a number you can change.',
      },
      { kind: 'mcq',
      ...
      { kind: 'decision', label: 'AT THE SHOP', title: 'The upgrade that pays off',
        ...
        options: [
          ...
          { t: 'A nicer pour-over kit', sub: 'Pour-over kit · pre-ground' },
        ],
      ...
      { kind: 'recall', label: 'BEFORE YOU GO',
        ...
        choices: [
          { t: 'An uneven grind', correct: true },
    ...
    cards: [
      ...
      { kind: 'decision', label: 'AT THE BREWER', title: 'Ground for filter',
        ...
        options: [
          ...
          { t: 'Buy it pre-ground and press it anyway', sub: 'Filter grind' },
        ],
    ...
    cards: [
      ...
      { kind: 'decision', label: 'AT THE BREWER', title: 'Sour, then bitter',
        ...
        options: [
          ...
          { t: 'Keep adjusting all three until it lands', sub: 'Grind and ratio together' },
        ],
      ...
      { kind: 'tastefix',
        tags: ['SOUR', 'THIN'],
        prompt: 'Your pour-over is sour and thin. What do you reach for first?',
        scenario: 'It drained faster than usual. Fresh beans, your usual ratio, water just off the boil.',
        choices: [
        ...
        explain: 'Sour and thin means the water left before it took enough with it. Finer grounds pack the bed tighter, slow the drawdown and pull more sweetness out. Cooler water is the reflex when a cup tastes sharp, and it does the opposite \u2014 slower extraction, and a cup that is sourer still.',
      },
      { kind: 'mcq',
        prompt: 'Finer grind generally makes a cup\u2026',
        choices: [
          { t: 'Extract faster, and turn bitter', correct: true },
  ...
  m4l6: {
    ...
    cards: [
      ...
      { kind: 'decision', label: 'AT THE SHELF', title: 'Grind it here?',
        ...
        options: [
          { t: 'Whole bean, plus the grinder', sub: 'Grind it yourself', correct: true },
          { t: 'Have them grind it now', sub: 'Ground at the counter' },
        ],
    ...
    cards: [
      ...
      { kind: 'decision', label: 'AT THE SHELF', title: 'One cup, every morning',
        ...
        options: [
          ...
          { t: 'The entry electric', sub: 'Electric, same price' },
        ],
        ...
        note: 'An electric grinder buys time, not grind quality. Know which one you are short of.' },
      { kind: 'recall', label: 'BEFORE YOU GO',
        question: 'You drink filter, but you might buy an espresso machine one day. What should that change about the grinder you buy now?',
        choices: [
          { t: 'Nothing \u2014 espresso wants a far pricier grinder anyway', correct: true },
          { t: 'Buy the espresso-capable one now and grow into it' },
          { t: 'Buy the cheapest grinder now and save the difference' },
        ],
        explain: 'Espresso needs a much finer, more even grind than filter, and the grinders that hold it start well above this budget. Buying \u201cespresso-capable\u201d cheaply is the common trap: it gets you a grinder that is worse at the filter you actually drink today, in exchange for espresso it still cannot pull.',
  ...
  m5l1: {
    ...
    cards: [
      { kind: 'predict', label: 'LESSON 1', title: 'The brew ratio',
        ...
        question: 'Double the coffee and the water together, and the cup gets ___',
        options: ['Stronger', 'No stronger at all'],
        a: 'No stronger at all',
      ...
      { kind: 'visual', label: 'VISUAL GUIDE', title: 'Weak to strong', visualGuide: 'ratio',
        caption: 'Find your baseline on this line first \u2014 everything else you tune from there.' },
      { kind: 'decision', label: 'AT THE BREWER', title: 'Clean but thin',
        ...
        options: [
          { t: '20g coffee, same water', sub: 'Ratio ≈ 1:16', correct: true },
          { t: 'Grind two steps finer', sub: 'Same 1:18 ratio' },
        ],
        right: 'Thin but not sour is a strength problem, not an extraction one. Move the ratio — 1:18 up to 1:16 — and leave grind alone.',
      ...
      { kind: 'tastefix',
        tags: ['HEAVY', 'TOO MUCH'],
        prompt: 'Your cup is thick and overwhelming \u2014 you can barely finish it. What first?',
        scenario: 'Nothing tastes harsh or dry. There is just too much of everything. You brewed it at 1:12.',
        choices: [
          { t: 'Add hot water to the cup', correct: true },
          { t: 'Grind coarser' },
          { t: 'Brew for less time' },
          { t: 'Switch to a lighter roast' },
        ],
        explain: 'Too strong is not the same as over-extracted. Nothing is harsh or drying, so the extraction was fine \u2014 there is simply too much coffee in the water. Adding water to the cup fixes strength without touching extraction, and tells you the next brew wants a ratio nearer 1:16. Grinding coarser is the common reflex, but it changes extraction instead and leaves the cup hollow as well as strong.',
      },
      { kind: 'mcq',
        prompt: 'A cup is sour and thin. Is more coffee the answer?',
        choices: [
          { t: 'No — grind finer instead', correct: true },
          { t: 'Yes, move to a tighter ratio' },
          { t: 'Yes, and use hotter water' },
          { t: 'Only if the water is soft' },
        ],
        explain: 'Sour means the water never got enough out of the grounds, and adding coffee gives it still more to get through \u2014 you land on sour and heavy. Thin but sweet is a strength problem the ratio fixes; sour is an extraction problem the grind fixes. Work out which of the two you have before you change anything.',
      },
      { kind: 'recall', label: 'BEFORE YOU GO',
  ...
  m5l2: {
    ...
    cards: [
      ...
      { kind: 'decision', label: 'IN THE KITCHEN', title: 'Blaming the beans',
        ...
        note: 'Filtered is not the same as soft, though. Water stripped of all its minerals extracts badly and tastes hollow — what you want gone is the chlorine, not everything.' },
      ...
      { kind: 'tastefix',
        ...
        explain: 'Minerals in the water are what grab flavour out of the grounds. Strip them all out and extraction has little to work with, so the cup reads thin and papery however good the beans are. Distilled and zero-TDS water is the common overshoot \u2014 clean is the goal, empty is not. Grinding finer chases the same emptiness with more surface area and usually turns it bitter without adding any sweetness.',
      },
  ...
  m5l4: {
    ...
    cards: [
      { kind: 'predict', label: 'LESSON 3', title: 'Extraction explained',
        body: 'Ratio sets how much coffee you start with. Extraction is how much of that coffee actually left the grounds and made it into the cup.',
        question: 'Of a roasted bean, the share that can dissolve in water at all is about ___',
        options: ['A third', 'Almost all of it'],
        a: 'A third',
        hold: 'Held. Most of a bean is never coming out \u2014 which is why the number you aim for is smaller than you\u2019d guess.' },
      { kind: 'concept', label: 'CONCEPT', title: 'What extraction is',
        ...
        paragraphs: [
          'Only some of a coffee ground is soluble at all. Of the bean\u2019s whole weight, you want roughly eighteen to twenty-two percent to end up in the cup.',
      ...
      { kind: 'concept', label: 'CONCEPT', title: 'Only part of a bean can dissolve',
        fill: ['At most about ', { a: '30%', o: ['30%', '75%'], label: 'The ceiling' }, ' of a bean will dissolve, so an 18\u201322% yield takes ', { a: 'most', o: ['most', 'a sliver'], label: 'Of what is there' }, ' of what is actually available.'],
        paragraphs: [
          'Grounds are mostly insoluble plant matter \u2014 cellulose that water cannot touch. Only around thirty percent of a roasted bean will ever dissolve, however long you brew it.',
          'That is why the target band sits where it does. Pulling eighteen to twenty-two percent means taking most of what is on offer, and leaving the last stubborn part behind on purpose.',
        ],
        meta: [['CEILING', '~30% soluble'], ['TARGET', '18\u201322%'], ['THE REST', 'Stays behind']],
      },
      { kind: 'concept', label: 'CONCEPT', title: 'Things leave the grounds in order',
        fill: ['Acids leave the grounds ', { a: 'first', o: ['first', 'last'], label: 'Acids' }, ', and the bitter compounds leave ', { a: 'last', o: ['last', 'first'], label: 'Bitter compounds' }, '.'],
        paragraphs: [
          'Extraction is not one flavour arriving all at once. Acids are small and quick, so they come out early. The sugars and the compounds that read as sweetness follow them.',
          'The heavy, bitter compounds are slowest \u2014 the last thing the water reaches. That order is why two brews at the same ratio taste different when one runs longer.',
        ],
      ...
      { kind: 'recall', label: 'BEFORE YOU GO',
        question: 'Two brews, same coffee and same grind. Which one extracts more?',
        choices: [
          { t: 'The one brewed with hotter water', correct: true },
          { t: 'The one poured off sooner' },
          { t: 'The one left completely still' },
        ],
        explain: 'Heat speeds dissolving, so at the same grind the hotter brew pulls more out. Cutting the brew short and never stirring both do the opposite \u2014 less time, less contact.',
        line: 'Grind, time, heat and ratio all move one number: how much dissolved.' },
    ],
    reward: {
      title: 'Extraction',
      summary: 'How much of the coffee actually made it into the water.',
      fact: 'Only about 30% of a bean can dissolve at all \u2014 and you want 18\u201322% of the bean\u2019s weight in the cup.',
      meta: [['CEILING', '~30% soluble'], ['TARGET', '18\u201322%'], ['FIRST OUT', 'Acids']],
    },
  },
    ...
    cards: [
      ...
      { kind: 'match',
        ...
        pairs: [
          ...
          { l: 'French press plunger', r: 'Heavy and oily' },
      ...
      { kind: 'recall', label: 'BEFORE YOU GO',
        ...
        choices: [
          { t: 'Switch to a metal filter', correct: true },
  ...
  m5l3: {
    ...
    cards: [
      ...
      { kind: 'concept', label: 'CONCEPT', title: 'The words for what you taste',
        fill: ['Brightness in a cup is ', { a: 'acidity', o: ['acidity', 'bitterness'], label: 'Brightness' }, '; its weight on your tongue is ', { a: 'body', o: ['body', 'finish'], label: 'Weight' }, '.'],
        paragraphs: [
          'Four words carry almost every tasting note you will read. Acidity is the bright, juicy liveliness \u2014 a good thing, and not the same as sour.',
          'Body \u2014 also called mouthfeel \u2014 is how heavy the cup sits on your tongue. Finish is what lingers after you swallow, and balance is how well all of it fits together: the word for a cup where nothing sticks out.',
        ],
      ...
      { kind: 'decision', label: 'AT THE BREWER', title: 'Bitter, and you want it fixed by morning',
        ...
        options: [
          ...
          { t: 'Coarser grind, cooler water, wider ratio', sub: 'Grind, ratio and water' },
        ],
      ...
      { kind: 'tastefix', tags: ['FLAT', 'HARSH'],
        prompt: 'Straight out of the kettle it tastes flat and a bit harsh. What do you do?',
        scenario: 'A recipe you have brewed a hundred times. Nothing about the brew changed.',
        choices: [
          { t: 'Wait \u2014 taste it again as it cools', correct: true },
          { t: 'Grind coarser next time' },
          { t: 'Cut it with hot water' },
          { t: 'Brew the next one cooler' },
        ],
        explain: 'Heat flattens what you can taste. Sweetness and acidity only really show themselves once a cup drops to around 50\u201360\u00b0C, which is why cuppers wait before they score anything. Judging at pouring temperature is the most common way people misdiagnose a brew that was fine \u2014 and then \u201cfix\u201d it into something worse.',
      },
      { kind: 'flavor',
        clue: 'Sharp and lively, with sweetness underneath and a finish that lingers.',
        prompt: 'Name what you are tasting',
        choices: [
          { t: 'Acidity' },
          { t: 'Sourness' },
          { t: 'Bitterness' },
        ...
        answer: 0,
        explain: 'Acidity is brightness, and it is what a good cup is supposed to have. The tell is what comes with it: sweetness underneath and a finish that lasts. Sourness is the flat, puckering version with nothing behind it. Mistaking good acidity for sourness and grinding finer to \u201cfix\u201d it is the most common way people flatten a cup that was already right.',
      },
      { kind: 'recall', label: 'BEFORE YOU GO',
        question: 'Which pair of symptoms means you should extract more?',
        choices: [
          { t: 'Bitter and dry' },
          { t: 'Sour and thin', correct: true },
          { t: 'Sweet and rounded' },
        ],
        explain: 'Sour and thin is under-extraction — there is more flavour still in the grounds, so pull harder, starting with a finer grind. Bitter and dry is the opposite problem and wants the dial moved the other way. Sweet and rounded means you are already there.',
  ...
  m5l7: {
    moduleLabel: 'MODULE 5 \u00b7 BREW',
    title: 'Espresso, in one small cup',
    points: 10, time: 5,
    cards: [
      { kind: 'predict', label: 'LESSON 6', title: 'Espresso, in one small cup',
        body: 'Every filter brew so far has let gravity do the work. This one uses force \u2014 and that single change explains everything else about it.',
        question: 'Espresso is\u2026',
        options: ['A dark roast', 'A brewing method'],
        a: 'A brewing method',
        hold: 'Held. The answer is the whole lesson.' },
      { kind: 'concept', label: 'CONCEPT', title: 'Pressure, not roast',
        fill: ['Espresso pushes water through the grounds at about ', { a: 'nine bars', o: ['nine bars', 'one bar'], label: 'Pressure' }, ' of pressure, in roughly ', { a: 'thirty seconds', o: ['thirty seconds', 'four minutes' ], label: 'Time' }, '.'],
        paragraphs: [
          'Espresso is not a bean and not a roast level \u2014 it is a brewing method. Around nine bars of pressure does in half a minute what a pour-over needs several minutes to do, so the cup comes out small, syrupy and several times stronger than drip.',
          'That pressure also whips oils and dissolved gas into the tan foam on top. That foam is the crema, and it settles within a minute or two.',
        ],
        meta: [['PRESSURE', '~9 bar'], ['TIME', '~28 sec'], ['TYPICAL SHOT', '18 g in, 36 g out']],
      },
      { kind: 'concept', label: 'CONCEPT', title: 'The puck does the work',
        fill: ['Grounds go into the ', { a: 'portafilter', o: ['portafilter', 'kettle'], label: 'The basket' }, ', and you press them flat \u2014 that press is the ', { a: 'tamp', o: ['tamp', 'bloom'], label: 'The press' }, '.'],
        paragraphs: [
          'The portafilter is the handled basket that locks into the machine. You grind into it, level the coffee, then tamp \u2014 press it flat and firm so the bed is even everywhere.',
          'Even matters because water is lazy: give it a crack or a soft spot and it rushes through that instead of the whole bed. That shortcut is channeling, and it is why a shot can run fast and still taste both sour and harsh.',
        ],
        meta: [['PORTAFILTER', 'Holds the coffee'], ['TAMP', 'Levels the bed'], ['CHANNELING', 'Water takes a shortcut']],
      },
      { kind: 'mcq',
        prompt: 'A shot sprays unevenly and tastes thin and harsh at once. The likely cause?',
        choices: [
          { t: 'Channeling through an uneven puck', correct: true },
          { t: 'The beans are too fresh' },
          { t: 'The tamp was pressed too hard' },
          { t: 'The water is running too hot' },
        ],
        explain: 'Uneven coffee lets water carve a channel \u2014 part of the bed is over-extracted, the rest is barely touched, so you taste both faults together.',
      },
      { kind: 'decision', label: 'AT THE MACHINE', title: 'The shot ran in eighteen seconds',
        scenario: 'Your shot gushed out in eighteen seconds and tastes sour and thin. Everything else is as it was yesterday.',
        question: 'What do you change?',
        options: [
          { t: 'Grind finer, pull again', sub: 'Slower shot', correct: true },
          { t: 'Raise the dose and the temperature', sub: 'More coffee, hotter water' },
        ],
        right: 'A fast shot means water met too little resistance. A finer grind slows it down and pulls more out \u2014 this is what dialling in means, and it is a daily job, not a one-off.',
        wrong: 'That may land a decent shot, but with two changes you will not know which one did it \u2014 and tomorrow the coffee will have aged again.',
        note: 'Hold the dose and the output steady. Grind is the only dial you turn.' },
      { kind: 'recall', label: 'BEFORE YOU GO',
        question: 'An espresso “cut” with a small, equal amount of warm milk is a\u2026',
        choices: [
          { t: 'Cortado', correct: true },
          { t: 'Long black' },
          { t: 'Ristretto' },
        ],
        explain: 'Cortado is Spanish for “cut” \u2014 espresso softened with just enough warm milk to take the edge off, and nothing like the volume of a latte.',
        line: 'Pressure makes it espresso. Everything else is what you pour on top.' },
    ],
    reward: {
      title: 'The Shot',
      summary: 'Why nine bars makes a different drink out of the same coffee.',
      fact: 'Espresso is a method, not a roast: ~9 bars for ~28 seconds, 18 g in and about 36 g out, with crema on top.',
      meta: [['PRESSURE', '~9 bar'], ['TIME', '~28 sec'], ['FAULT TO KNOW', 'Channeling']],
    },
  },
  m5l6: {
    moduleLabel: 'MODULE 5 · BREW',
    ...
    points: 10, time: 5,
    cards: [
      { kind: 'predict', label: 'LESSON 7', title: 'Your first good cup',
      ...
      { kind: 'practical', tag: 'WHAT YOU NEED', title: 'On the counter',
        paragraphs: [
          'Beans roasted within the month. A grinder. A scale. A pour-over brewer with its filter. Water just off the boil.',
      ...
      { kind: 'tastefix', tags: ['STALLED', 'HARSH'],
        prompt: 'The drawdown stalled and the cup is harsh and drying. What do you change?',
        ...
        explain: 'A bed that stops draining is ground too fine: the water sits in contact far past the time the recipe assumed and keeps stripping out the harsh, drying compounds. Coarser lets it flow again. Pouring faster is the reflex, but it just floods a bed that already cannot drain \u2014 you get an overflowing brewer and the same bitter cup.',
      },
      { kind: 'decision', label: 'AT THE BREWER', title: 'Drinkable, but flat',
        ...
        options: [
          ...
          { t: 'Change the grind, ratio and water together', sub: 'Two changes at once' },
        ],
...
// The collectible catalogue. Array ORDER is the catalogue number printed on
// each card, so entries are never re-sorted — only appended.
//
// An entry carries identity only: `id`, `kind` (its art + tint), `unlock`, and
// the `earned` first-paint seed that window.syncCollection() overwrites from
// live progress. A card's WORDS are not stored here — syncCardText() below
// copies them from the one place they are authored: the lesson's `reward`, or
// MODULE_REWARDS for the five module Field Guides. One card, one text — so the
...
const COLLECTIBLES = [
  ...
  { id: 'c-m3l1', earned: false, unlock: { lesson: 'm3l1' }, kind: 'roastscale' },
  { id: 'c-m3l2', earned: false, unlock: { lesson: 'm3l2' }, kind: 'crack' },
  { id: 'c-m3l3', earned: false, unlock: { lesson: 'm3l3' }, kind: 'calendar' },
  { id: 'c-m4l1', earned: false, unlock: { lesson: 'm4l1' }, kind: 'particles' },
  { id: 'c-m4l2', earned: false, unlock: { lesson: 'm4l2' }, kind: 'burrblade' },
  { id: 'c-m4l3', earned: false, unlock: { lesson: 'm4l3' }, kind: 'grinddial' },
  ...
  { id: 'c-m1l6', earned: false, unlock: { lesson: 'm1l6' }, kind: 'varieties' },
  { id: 'c-m2l4', earned: false, unlock: { lesson: 'm2l4' }, kind: 'drying' },
  { id: 'c-m2l5', earned: false, unlock: { lesson: 'm2l5' }, kind: 'anaerobic' },
  { id: 'c-m2l6', earned: false, unlock: { lesson: 'm2l6' }, kind: 'decaf' },
  { id: 'c-m3l4', earned: false, unlock: { lesson: 'm3l4' }, kind: 'roastcurve' },
  { id: 'c-m3l5', earned: false, unlock: { lesson: 'm3l5' }, kind: 'lightdark' },
  { id: 'c-m3l6', earned: false, unlock: { lesson: 'm3l6' }, kind: 'caffeine' },
  { id: 'c-m4l5', earned: false, unlock: { lesson: 'm4l5' }, kind: 'grindbrewer' },
  { id: 'c-m4l6', earned: false, unlock: { lesson: 'm4l6' }, kind: 'hourglass' },
  { id: 'c-m4l7', earned: false, unlock: { lesson: 'm4l7' }, kind: 'burrs' },
  { id: 'c-m5l4', earned: false, unlock: { lesson: 'm5l4' }, kind: 'extraction' },
  { id: 'c-m5l5', earned: false, unlock: { lesson: 'm5l5' }, kind: 'filter' },
  { id: 'c-m5l7', earned: false, unlock: { lesson: 'm5l7' }, kind: 'shot' },
...
// A module card's spec rows are computed, not authored: the lesson count reads
// off MODULES, so adding a lesson can never leave a stale number on the card.
// (Module 5 still said "6 lessons" after espresso was written.)
MODULES.forEach((m) => {
  const r = MODULE_REWARDS[m.id];
  if (!r) return;
  r.meta = [
    ['MODULE', String(m.n).padStart(2, '0') + ' · ' + m.title],
    ['LESSONS', String(m.lessons.length)],
    ['EARNED BY', 'Finishing ' + m.title],
  ];
});
...
function syncCardText() {
  COLLECTIBLES.forEach((c) => {
    ...
    const src = c.unlock.lesson ? (LESSONS[c.unlock.lesson] || {}).reward
              ...
              : null;
    if (!src) return;
    ['title', 'summary', 'fact', 'meta'].forEach((k) => {
      if (src[k] != null) c[k] = src[k];
    });
  });
}
syncCardText();
...
// Identity lives here; title/summary/fact are copied from VISUAL_GUIDE_CONTENT by
...
// The display name is user-set (onboarding Name screen, Settings → Name) and
// persisted in cq-user; hydrate it here so module-scope readers see it at load.
try { const _u = JSON.parse(localStorage.getItem('cq-user')); if (_u && typeof _u.name === 'string') window.USER.name = _u.name; } catch (e) {}
window.USER.initial = window.USER.name ? window.USER.name.charAt(0).toLowerCase() : '·';
