...
/// The sentinel colours a card's art is written in, and the token each maps
/// back to: an `.svg` cannot hold `var(--sage)`, so the extractor writes a
/// magenta literal that is visibly wrong if it ever slips through unmapped.
///
/// Two families: the mood tokens flip and come from [MoodColors]; the
/// `--art-*` family is fixed in both moods and comes from [ArtColors].
@immutable
class _ArtColorMapper extends ColorMapper {
  ...
  /// The mood-independent half, by token name rather than value, so the
  /// palette stays in [ArtColors] alone. Keyed by the packed ARGB value
  /// because `Color` overrides `==` and cannot key a const map.
...
/// Draws one collectible's own illustration, filling the box it is given; the
/// only way to draw one, since the arts paint in sentinel colours that a bare
/// `SvgPicture` would render as magenta.
///
/// The art is `width: 100%; height: 100%` in its slot; a kind the design has
/// not drawn takes [fallback], the module's mark, at a fixed [fallbackSize].
