...
/// What a rendered card reports to its host — they always travel together,
/// and a kind that reports nothing still takes the whole value.
typedef CardHost = ({
  CardSolved onSolved,
...
/// always returns a widget — a card this app cannot draw is not a state it can
/// be in (#418). [seed] fixes the choice order for this attempt
/// (`card_seed.dart`); [host] carries the guess loop the opening card opens.
Widget contentCardView(
  ContentCard card, {
  required AppLocalizations strings,
  required int seed,
  required CardHost host,
}) {
  final held = host.guess.held;

  return switch (card) {
    final PredictCard predict => PredictCardView(
      card: predict,
      options: shuffledBySeed(predict.options, seed),
      onContinue: host.onContinue,
      onGuess: host.guess.onGuess,
    ),
    ...
    final QuizCard quiz => GradedPicker(
      options: shuffledBySeed(_quizOptions(strings, quiz), seed),
      copy: _quizCopy(quiz),
      onSolved: host.onSolved,
      onContinue: host.onContinue,
    ),
...
List<ChoiceOption> _quizOptions(AppLocalizations strings, QuizCard card) => [
  ChoiceOption(text: strings.quizTrue, isCorrect: card.answer),
  ChoiceOption(text: strings.quizFalse, isCorrect: !card.answer),
];
...
PickerCopy _decisionCopy(DecisionCard card) => PickerCopy(
  ...
  verdict: (strings, {required wasCorrect}) =>
      wasCorrect ? strings.decisionGoodCall : strings.decisionBackfire,
  footnote: card.note,
);
