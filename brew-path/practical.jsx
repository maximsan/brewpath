...
// saved / flashcards):
//
//   1. PracticalCard    — "what this means when you brew" (concept → practice)
//   2. TasteFixCard     — a lightweight scenario mini-game: my cup tastes X,
//                         what do I change first?
//   3. TrainingCard     — reusable visual reference: Roast / Grind / Extraction
//                         / Ratio. Rendered full inside lessons + the card
//                         sheet, and as a compact thumb in the collection grid.
//
// Nothing here invents a new engine — practical + tastefix are just card kinds
// the LessonPlayer already knows how to advance, and TrainingCard is plain
...
// ───────────────────────────────────────────────────────────
// PRACTICAL CARD
// A concept card's plainer cousin: what the idea means the next time you
// actually brew. Same rhythm as ConceptCard (eyebrow → title → prose →
// Continue) but with an accent "in practice" voice and an optional highlighted
// rule-of-thumb line — the one sentence worth remembering at the counter.
// card = { kind:'practical', title, paragraphs:[], note?, tag? }
// ───────────────────────────────────────────────────────────
function PracticalCard({ card, onContinue, onTermTap }) {
  ...
  return (
    <div className="px-24" style={{ display: 'flex', flexDirection: 'column', flex: '1 0 auto', minHeight: 600 }}>
      <div className="smallcaps" style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14, color: 'var(--accent)' }}>
        <TuneMark size={15} color="var(--accent)"/> {card.tag || 'IN PRACTICE'}
      </div>
      ...
      <div style={{ marginTop: 20 }}>
        {(card.paragraphs || []).map((p, i) => (
          <p key={i} style={{
            ...
            marginTop: i === 0 ? 0 : 16, marginBottom: 0, textWrap: 'pretty',
          }}>{link(p)}</p>
        ))}
      </div>
      ...
      {card.note && (
        <div style={{
          marginTop: 26, borderRadius: 14, padding: '16px 18px',
          background: 'color-mix(in oklab, var(--accent) 8%, var(--surface))',
          border: '1px solid color-mix(in oklab, var(--accent) 26%, var(--rule))',
          display: 'flex', gap: 13, alignItems: 'flex-start',
        }}>
          <span style={{ flexShrink: 0, marginTop: 1 }}><BrewCup size={19} color="var(--accent)"/></span>
          <div>
            <div className="ff-mono" style={{ fontSize: 'var(--t-micro)', letterSpacing: '0.16em', textTransform: 'uppercase', color: 'var(--accent)', marginBottom: 5 }}>RULE OF THUMB</div>
            <p style={{ margin: 0, fontSize: 'var(--t-body)', lineHeight: 1.5, color: 'var(--ink)', textWrap: 'pretty' }}>{card.note}</p>
          </div>
        </div>
      )}
    ...
    <div className="px-24" style={{ display: 'flex', flexDirection: 'column', flex: '1 0 auto', minHeight: 600 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginTop: 24 }}>
        {card.choices.map((c, i) => {
          ...
          return (
            <button key={i} className={cls} disabled={picked !== null} onClick={() => handlePick(i)}>{c.t}</button>
          );
        })}
      </div>
...
// Literal bean browns — read as coffee in both light + dark themes.
const ROAST_BEANS = ['#C68A4E', '#9A5A31', '#3E2419'];
...
function GrindDots({ level }) {
  ...
  return (
    <span style={{ width: 30, height: 30, borderRadius: 8, display: 'block', background: 'var(--surface-2)', border: '1px solid var(--rule)' }}>
...
const TRAINING = {
  ...
  extraction: {
    ...
    body: () => (
      <SpectrumBar
        colors={['color-mix(in oklab, var(--berry) 55%, var(--surface))', 'color-mix(in oklab, var(--sage) 65%, var(--surface))', '#3E2419']}
        marker={0.5}
        stops={[
...
function TrainingCard({ variant, inSheet = false }) {
  ...
  return (
    <div style={{
      ...
      <div style={{ marginTop: 18 }}>{t.body()}</div>
    </div>
  );
}
...
function VisualLessonCard({ card, onContinue, saved, onToggleSave }) {
  ...
  return (
    <div className="px-24" style={{ display: 'flex', flexDirection: 'column', flex: '1 0 auto', minHeight: 600 }}>
      ...
      <TrainingCard variant={card.variant}/>
      {onToggleSave && (
...
function TrainingThumb({ variant }) {
  ...
  if (variant === 'extraction') {
    return (
      <svg viewBox="0 0 100 100" style={{ width: '100%', height: '100%' }}>
        ...
        <rect x="61.4" y="46" width="22.6" height="10" rx="2" fill="#3E2419"/>
  ...
  return (
    <svg viewBox="0 0 100 100" style={{ width: '100%', height: '100%' }}>
      ...
      <rect x="38.6" y="46" width="22.8" height="10" fill="#9A5A31"/>
