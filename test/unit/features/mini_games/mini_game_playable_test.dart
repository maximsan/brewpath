...
// `playableMiniGameIds` is an allowlist on purpose (#121), and its price is a
// game that joins the catalog, never joins the list, and sits on the shelf
// offering a dead button (#311). So the rule is stated the other way round: a
// game with rounds to play is playable unless someone named a reason it is not,
// which a build can check. Asserted against the real shipped banks — a fixture
// would only prove the rule holds over invented data.
void main() {
  ...
  test('nothing playable opens a run with no rounds in it', () {
    ...
    // learner a run with nothing to show them. It used to also catch a bank
    // this build could not draw, which the renderer check (#418) made
  ...
  test('the free tier can reach every round it advertises', () {
    // ADR-0007 promises the free learner three games. All three were authored
    // the whole time; one had no renderer, so free practice advertised 18
    // rounds and reached 11 (#123, #308). Asserted as a count, because that is
    // what the learner experiences and what the ADR promises: it fails if a
    // free game loses its renderer, leaves the playable set, or the free set is
    // re-picked.
